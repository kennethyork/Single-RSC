...
public class BotAPI {
     ...
     *
     * @return true when the bank is open or the player is making progress toward
     *         a banker; false when no banker can be reached yet
     */
    public boolean openBank() {
        ...
        if (!getPlayer().nextTo(banker)) {
            ...
            if (tryOpenBankDoor(banker)) {
                return true;
            }

            // A route request is progress even when the local pathfinder has to
            // fall back to a direct segment. Repeated calls continue the journey
            // as new map regions load instead of aborting after four attempts.
    ...
    /** Opens a closed door that is blocking the final approach to a bank. */
    private boolean tryOpenBankDoor(NPC banker) {
        long now = System.currentTimeMillis();
        if (now - bankLastProgressAt < 1500L || now - bankLastDoorActionAt < 1500L) {
            return false;
        }
        ...
        for (GameObject object : RegionManager.getLocalObjects(getPlayer())) {
            if (object == null || object.isRemoved() || object.getType() != 1) {
                continue;
            }
            DoorDef definition = object.getDoorDef();
            if (definition == null || !"open".equalsIgnoreCase(definition.getCommandFirst())) {
                continue;
            }
