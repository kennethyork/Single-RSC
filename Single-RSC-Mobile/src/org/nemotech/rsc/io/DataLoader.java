...
public class DataLoader {
    ...
    private <T> T load(String ident, Type type) throws Exception {
        String path = files.get(ident);
        InputStream inputStream = Util.openFile(path);
        InputStreamReader inputStreamReader = new InputStreamReader(inputStream);
        //System.out.print("loaded: " + ident);
        T result = (T) gson.fromJson(new JsonReader(inputStreamReader), type);
        inputStream.close();
        return result;
    }
    ...
    private String wrap(String file) {
        return Constants.CACHE_DIRECTORY + "data/" + file + ".json";
    }
