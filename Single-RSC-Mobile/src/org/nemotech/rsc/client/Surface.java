...
import org.nemotech.rsc.android.AndroidPlatform;
import org.nemotech.rsc.android.GameBridge;

/**
 * Software rendering surface. All drawing operates on imagePixelArray (int[]).
 * On Android, the pixel array is blitted to a Bitmap via GameSurfaceView.
 */
public class Surface {
    
	public Sprite[] sprites;
	/** Sprite archive data loaded into memory (since Android assets aren't random-access ZipFiles). */
	private java.util.Map<String, byte[]> spriteArchiveEntries;
	...
	public Surface(int width, int height, int k) {
		...
		// Load sprite archive from assets into memory
		loadSpriteArchive();
	}
	...
	public void drawSleepWord(int x, int y, Sprite sleepy) {
		try {
            ...
            e.printStackTrace();
            throw new RuntimeException("Error drawing sleepy", e);
        }
	}
    
	private void loadSpriteArchive() {
		spriteArchiveEntries = new java.util.HashMap<>();
		try {
			InputStream is = AndroidPlatform.openAsset("graphics-members.zip");
			...
			while ((entry = zis.getNextEntry()) != null) {
				...
				spriteArchiveEntries.put(entry.getName(), baos.toByteArray());
				zis.closeEntry();
			}
			zis.close();
			System.out.println("SPRITE_ARCHIVE: Loaded " + spriteArchiveEntries.size() + " entries from graphics-members.zip");
		} catch (IOException e) {
			System.err.println("SPRITE_ARCHIVE: Failed to load graphics-members.zip");
			e.printStackTrace();
		}
	}
	...
	public boolean loadSprite(int id, String packageName) {
		try {
			byte[] data = spriteArchiveEntries.get(String.valueOf(id));
			if (data == null) {
				System.err.println("Missing sprite: " + id);
				return false;
			}
			sprites[id] = Sprite.unpack(ByteBuffer.wrap(data));
			return true;
		}
		catch(Exception e) {
			e.printStackTrace();
    ...
    /**
     * Draw the pixel buffer to screen. On Android, sends pixels to GameSurfaceView.
     */
    public void draw(int x, int y) {
        GameBridge.surfaceView.renderFrame(imagePixelArray, width2, height2);
    }
    ...
    public void drawSprite(int x, int y, int picture) {
    	try {
            ...
            e.printStackTrace();
            throw new RuntimeException("Error drawing: " + picture, e);
        }
    }
    ...
    public void spriteClipping(int i, int j, int k, int l, int i1, int overlay, int k1, int l1, boolean flag) {
        try {
            ...
            System.out.println("SPRITE_CLIP_ERROR sprite=" + i1 + " at=(" + i + "," + j + ") size=" + k + "x" + l + " overlay=0x" + Integer.toHexString(overlay) + " skin=0x" + Integer.toHexString(k1) + " err=" + e.getMessage());
            e.printStackTrace();
        }
    }
    ...
    private void spritePlotTransparent(int ai[], int ai1[], int i, int j, int k, int l, int i1, int j1, int k1, int l1, int i2, int overlay, int k2, int l2, int i3) {
        ...
        try {
            ...
            System.out.println("PLOT13_ERROR: " + _ex.getMessage());
            _ex.printStackTrace();
        }
    }
    ...
    private void spritePlotTransparent(int ai[], int ai1[], int i, int j, int k, int l, int i1, int j1, int k1, int l1, int i2, int overlay, int k2, int l2, int i3, int j3) {
        ...
        try {
            ...
            System.out.println("PLOT16_ERROR: " + _ex.getMessage());
            _ex.printStackTrace();
        }
    }
    ...
    // loadFont removed - fonts are loaded from .jf files in JAG archives via createFont()

    // drawLetter removed - fonts are loaded from .jf files in JAG archives via createFont()
