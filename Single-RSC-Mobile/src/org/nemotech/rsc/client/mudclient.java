...
public class mudclient extends Shell {
    ...
    private int graphics; // unused, kept for compatibility
    public int regionX;
    public int regionY;
    ...
    private void drawUiTabMusic(boolean nomenus) {
        ...
        try {
            if (Constants.IS_ANDROID) {
                String[] allFiles = org.nemotech.rsc.android.AndroidPlatform.listAssets("audio/music");
                java.util.List<String> midFiles = new java.util.ArrayList<>();
                for (String name : allFiles) {
                    if (name.endsWith(".mid")) midFiles.add(name);
                }
                songNames = midFiles.toArray(new String[0]);
            } else {
                File musicFolder = new File(Constants.CACHE_DIRECTORY + "audio/music");
                File[] files = musicFolder.listFiles((File dir, String name) -> name.endsWith(".mid"));
                songNames = new String[files != null ? files.length : 0];
                if (files != null) {
                    Arrays.sort(files);
                    for (int i = 0; i < files.length; i++) songNames[i] = files[i].getName();
                }
            }
        } catch (Exception e) {
        ...
        for(int i = 0; i < songNames.length; i++) {
            panelMusic.addListEntry(controlListMusic, i, Util.capitalizeWord(songNames[i].replace(".mid", "").replace("_", " ")));
        }
    ...
    private void handleGameInput() {
        ...
        // Apply touch drag rotation
        int rotDelta = super.cameraRotationDelta;
        if (rotDelta != 0) {
            super.cameraRotationDelta = 0;
            if (optionCameraModeAuto) {
                // In auto mode, change discrete angle
                if (rotDelta > 0) {
                    cameraAngle = cameraAngle + 1 & 7;
                } else {
                    cameraAngle = cameraAngle + 7 & 7;
                ...
                cameraRotation = cameraRotation + rotDelta & 255;
            }
        }
        int zoomTarget;
        if (fogOfWar) {
        ...
        zoomTarget += super.cameraZoomOffset;
        zoomTarget = Math.max(200, Math.min(750, zoomTarget));
        if (cameraZoom < zoomTarget) {
            cameraZoom = Math.min(cameraZoom + 4, zoomTarget);
        } else if (cameraZoom > zoomTarget) {
            cameraZoom = Math.max(cameraZoom - 4, zoomTarget);
        }
    ...
    private int[] tempDimensionWH;

    public void resetLoginVars() {
        loginScreen = 0;
        loggedIn = 0;
        logoutTimeout = 0;
        tempDimensionWH = new int[]{gameWidth, gameHeight};
        // On mobile, no window resizing needed
    }
    ...
    private void drawDialogShop() {
        ...
        if (selectedItemType != -1) {
            ...
            if (getInventoryCount(selectedItemType) > 0) {
                ...
                if (itemPrice == 0) {
                    itemPrice = EntityManager.getItem(selectedItemType).getPrice() / 2; //: needs to be fixed
                }
    ...
    protected void draw() {
        ...
        try {
            if (loggedIn == 0) {
                ...
                drawGame();
                // Resizing not supported on mobile
